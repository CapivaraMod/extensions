...
class HomepageFile extends BuildFile {
  constructor(
    ...
    this.host =
      mode === "development"
        ? "http://localhost:8000/"
        : "https://capivaramod.github.io/extensions/";
  }
  ...
  getRunExtensionURL(extensionSlug) {
    return `https://capivaramod.github.io/editor?extension=${this.getFullExtensionURL(
      extensionSlug
    )}`;
  }
  ...
  getRunSampleURL(sampleFile) {
    ...
    return `https://capivaramod.github.io/editor?project_url=${this.host}${path}`;
  }
...
class SitemapFile extends BuildFile {
  ...
  read() {
    ...
    xml += Object.keys(this.build.files)
      ...
      .map((path) => `https://capivaramod.github.io/extensions${path}`)
...
class Builder {
  ...
  async build() {
    ...
    for (const [oldPath, newPath] of Object.entries(compatibilityAliases)) {
      ...
      if (!target) {
        console.warn(
          `Skipping compatibility alias ${oldPath}: target ${newPath} does not exist`
        );
        continue;
      }
